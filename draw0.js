...
function drawTarget(circles=10,gap=20){     
  ...
  function randomHexagon() {
    const sideLength = Math.random() * 50 + 30; // Náhodná délka strany
    const x = Math.random() * (canvas.width - sideLength);
    const y = Math.random() * (canvas.height - sideLength);
    const color = `rgb(${Math.random() * 255}, ${Math.random() * 255}, ${Math.random() * 255})`;
    drawHexagon(x, y, sideLength, color);
  }
  
  function drawHexagon(x, y, sideLength, color) {
    ctx.fillStyle = color;
    ctx.beginPath();
    const angle = (Math.PI / 3) * 2;
    for (let i = 0; i < 6; i++) {
      const xPoint = x + sideLength * Math.cos(i * angle);
      const yPoint = y + sideLength * Math.sin(i * angle);
  ...
  function randomSquare() {
    ...
    const y = Math.random() * (canvas.height - sideLength);
    const color = `rgb(${Math.random() * 255}, ${Math.random() * 255}, ${Math.random() * 255})`;
    drawSquare(x, y, sideLength, color);
  }
